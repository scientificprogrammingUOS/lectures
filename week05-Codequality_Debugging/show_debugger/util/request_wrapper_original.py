...
# 
#         sys.exit(1)
#     elif (int(headers['x-ratelimit-remaining']) < 50 and int(
#             headers['x-ratelimit-remaining']) % 10 == 0) or self.num_calls == 0:
...
#         sleeptime = (
#                     datetime.utcfromtimestamp(int(headers['x-ratelimit-reset']) + 2 * 60 * 60) - datetime.now()).seconds
