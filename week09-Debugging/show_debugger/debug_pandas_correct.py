...
def find_best_defender(all_pokemon, effectivity_attack):
    all_pkmn_bkp = all_pokemon.copy()
    all_pokemon['Type'] = all_pokemon[['Type 1', 'Type 2']].apply(lambda row: np.nan if pd.isna(row['Type 2']) else (row['Type 1']+' '+row['Type 2'] if row['Type 1'] < row['Type 2'] else row['Type 2']+' '+row['Type 1']),axis=1)
    type_nums = all_pokemon.groupby(['Type'])['Name'].count().reset_index()
    type_nums = type_nums.rename({'Name': 'Count'}, axis='columns')
    common_type_1, common_type_2 =  list(type_nums.iloc[type_nums['Count'].idxmax(axis=0), :][['Type']])[0].split(' ')
    ...
    best_defenders = all_pkmn_bkp[((all_pokemon['Type 1'] == defender_type_1) & (all_pokemon['Type 2'] == defender_type_2)) | ((all_pokemon['Type 1'] == defender_type_2) & (all_pokemon['Type 2'] == defender_type_1))]
    return best_defenders.sort_values(by='Total', ascending=False).iloc[0]



all_pokemon = pd.read_csv("data/Pokemon.csv", index_col=0)
effectivity_attack = pd.read_csv("data/Effectivity_table.csv", index_col=0)
